...
// 累计左右轮平均脉冲，Distance_Measure 读取后清零
...
int16 left_speed = 0;       // 左轮速度反馈，单位为本周期编码器脉冲
int16 right_speed = 0;      // 右轮速度反馈，单位为本周期编码器脉冲
int16 left_speed_raw = 0;   // 左轮原始脉冲，已按当前接线映射到左轮
int16 right_speed_raw = 0;  // 右轮原始脉冲，右侧取负用于统一前进方向
...
// 读取并清空本周期编码器计数，通常由控制环周期性调用
void encoder_update(void)
{
    left_speed_raw  = encoder_get_count(ENCODER_QUAD_2); // 读取并修正左侧方向
