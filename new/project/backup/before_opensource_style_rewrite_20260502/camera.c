...
// 基点行存在多组边界时，优先选择与上一帧中线连续、宽度合理的那一组。
#define CAMERA_JIDIAN_CAND_MAX           12
#define CAMERA_JIDIAN_MID_SCORE_K        4
#define CAMERA_JIDIAN_WIDTH_SCORE_K      1

// 邻近赛道防串线：一侧边界突跳到旁边白色赛道时，优先拒绝跳变更大的那一侧。
...
// 质量门控的路线预瞄：把S弯与直线接圆弯都放在统一路线层处理，避免低置信度下突然翻向。
...
#define CAMERA_RING_CONFIRM_FRAMES       4
#define CAMERA_RING_KEEP_SCORE_TH        4
#define CAMERA_RING_LOST_FRAMES          8
#define CAMERA_RING_ENTER_FRAMES         30
#define CAMERA_RING_INSIDE_FRAMES        52
#define CAMERA_RING_EXIT_FRAMES          26
#define CAMERA_RING_MIN_CONF             36
#define CAMERA_RING_WIDTH_OPEN_ADD       18
#define CAMERA_RING_OPEN_BIAS_TH         10
#define CAMERA_RING_STRAIGHT_PREVIEW_MAX 16
#define CAMERA_RING_FAR_PREVIEW_MAX      36
#define CAMERA_RING_STRONG_SCORE_TH      18
#define CAMERA_RING_STRONG_CONF_MIN      30
#define CAMERA_RING_STRONG_PREVIEW_MAX   24
#define CAMERA_RING_STRONG_FAR_MAX       40
#define CAMERA_RING_MID_BIAS_MIN         7
#define CAMERA_RING_MID_BIAS_MAX         18
#define CAMERA_RING_PREVIEW_BIAS         14
...
static int8 camera_ring_dir = 0;
static uint8 camera_ring_state = CAMERA_RING_NONE;
static uint8 camera_ring_score = 0;
...
static uint8 find_jidian_pair(int row, int *left_out, int *right_out)
{
    int right_cands[CAMERA_JIDIAN_CAND_MAX];
    int right_cnt;
    int seed_mid;
    int seed_width;
    int best_left;
    ...
    for (x = 2; x <= MT9V03X_W - 3; x++)
    {
        if (is_right_transition(row, x))
        {
            if (right_cnt < CAMERA_JIDIAN_CAND_MAX)
            {
                right_cands[right_cnt++] = x;
            }
            else
            {
                break;
    ...
    seed_mid = final_mid_init ? (int)final_mid_line : MID_W;
    seed_width = (int)right_jidian - (int)left_jidian;
    if ((seed_width < CAMERA_WIDTH_MIN) || (seed_width > CAMERA_WIDTH_MAX))
    {
        seed_width = estimate_half_width(row) * 2;
    }
    ...
    for (x = 2; x <= MT9V03X_W - 3; x++)
    {
        ...
        {
            right_x = right_cands[i];
            if (right_x <= left_x + CAMERA_EDGE_MIN_GAP)
            {
                continue;
            }
            ...
            mid_now = (left_x + right_x) / 2;
            score = abs_i(mid_now - seed_mid) * CAMERA_JIDIAN_MID_SCORE_K +
                    abs_i(width_now - seed_width) * CAMERA_JIDIAN_WIDTH_SCORE_K;
...
void find_jidian(void)
{
    ...
    if (!find_jidian_pair(y, &left_x, &right_x))
    {
...
void image_deal(void)
{
    ...
    ring_strong_guard = ((ring_score_tmp >= CAMERA_RING_STRONG_SCORE_TH) &&
                         (camera_confidence >= CAMERA_RING_STRONG_CONF_MIN) &&
                         (abs_i(ring_preview_tmp) <= CAMERA_RING_STRONG_PREVIEW_MAX) &&
                         (abs_i(ring_far_tmp) <= CAMERA_RING_STRONG_FAR_MAX)) ? 1 : 0;

    ring_entry_dir = 0;
    if (((camera_confidence >= CAMERA_RING_MIN_CONF) && ring_curve_guard) || ring_strong_guard)
    {
    ...
    if (ring_hold_state == CAMERA_RING_NONE)
    {
        if (ring_entry_dir != 0)
        {
            ...
            {
                if (ring_strong_guard)
                    ring_candidate_frames = CAMERA_RING_CONFIRM_FRAMES;
            ...
            else
            {
                ...
                ring_candidate_frames = ring_strong_guard ? CAMERA_RING_CONFIRM_FRAMES : 1;
            }
    ...
    else
    {
        ...
        else if (ring_curve_guard && (ring_score_tmp >= CAMERA_RING_KEEP_SCORE_TH) && (camera_confidence >= 25))
        {
            ring_keep_ok = 1;
        }
    ...
    camera_ring_score = (uint8)clamp_i(ring_score_tmp, 0, 100);
}
...
uint8 find_mid_line_weight(void)
{
    ...
    preview_far_out = clamp_i(pre_far, -90, 90);
    route_mode_tmp = 0;
    ...
    if ((ring_state_tmp != CAMERA_RING_NONE) && (ring_dir_tmp != 0))
    {
        ...
        {
            ring_bias = CAMERA_RING_MID_BIAS_MIN + ring_score_tmp / 3;
        }
        else if (ring_state_tmp == CAMERA_RING_INSIDE)
        {
            ring_bias = CAMERA_RING_MID_BIAS_MIN + 4 + ring_score_tmp / 4;
        }
        else
        {
        ...
        preview_tmp = pre_mid + ring_dir_tmp * CAMERA_RING_PREVIEW_BIAS;
    }
    else
    {
    ...
    else
    {
        ...
        final_mid_filtered_x8 = (int)(((int32)final_mid_filtered_x8 * (100 - CAMERA_MID_FILT_ALPHA_PCT) +
                                      target_mid_x8 * CAMERA_MID_FILT_ALPHA_PCT + 50) / 100);
    }
