...
#define DUAL_PD_LAT_D_BASE               0.28f
#define DUAL_PD_LAT_D_CURVE_ADD          0.28f
#define DUAL_PD_HEAD_P_CURVE_ADD         0.30f
#define DUAL_PD_HEAD_D_GAIN              0.88f
#define DUAL_PD_GYRO_D_CURVE_ADD         0.18f
#define DUAL_PD_EXIT_HEAD_KEEP           0.65f

#define STRAIGHT_LOCK_PREVIEW_TH         2.4f
#define STRAIGHT_LOCK_DPV_TH             0.45f
#define STRAIGHT_LOCK_ERR_TH             5.0f
#define STRAIGHT_LOCK_HOLD_CYCLES        12
#define STRAIGHT_LOCK_KP_MUL             0.68f
#define STRAIGHT_LOCK_KD_MUL             1.35f
#define STRAIGHT_LOCK_HEAD_MUL           0.16f
#define STRAIGHT_LOCK_STEP_MUL           0.55f
#define STRAIGHT_LOCK_RAW_DECAY          0.62f

#define S_CURVE_SPEED_KEEP               0.70f
#define S_CURVE_LAT_MUL                  0.68f
#define S_CURVE_FAR_BLEND                0.55f

#define RING_SPEED_KEEP                  0.58f
#define RING_LAT_MUL                     0.72f
#define RING_HEAD_BIAS                   2.8f

#define LOW_CONF_TH                      25
#define LOW_CONF_ERR_MUL                 0.55f
#define LOW_CONF_PREVIEW_MUL             0.45f
#define LOW_CONF_STEP_MUL                0.55f
#define LOW_CONF_STEER_LIMIT_MUL         0.62f
#define LOW_CONF_SPEED_KEEP              0.55f
...
#define OUTSIDE_SPEED_KEEP               0.80f
#define OUTSIDE_HOLD_CYCLES              8
...
volatile int16 control_debug_steer_x10 = 0;
volatile int16 control_debug_target_pulses = 0;
volatile int16 control_debug_left_target = 0;
volatile int16 control_debug_right_target = 0;
volatile uint8 control_debug_flags = 0;

// 基础速度（0~90）
int target_speed_base = 30;
...
void control_loop(void)
{
    ...
    ring_dir = camera_ring_dir;
    s_curve_flag = camera_s_curve_flag;
    low_conf = (camera_confidence < LOW_CONF_TH) ? 1 : 0;
    if (low_conf)
    {
        ring_dir = 0;
        s_curve_flag = 0;
    ...
    preview_far_filtered = preview_far_filtered * (1.0f - PREVIEW_LPF_ALPHA) + preview_far_raw * PREVIEW_LPF_ALPHA;
    if (low_conf)
    {
        preview_filtered *= LOW_CONF_PREVIEW_MUL;
        preview_far_filtered *= LOW_CONF_PREVIEW_MUL;
    }
    ...
    head_preview = preview_filtered;
    if (s_curve_flag)
    {
        head_preview = preview_filtered * (1.0f - S_CURVE_FAR_BLEND) + preview_far_filtered * S_CURVE_FAR_BLEND;
        kp_now *= S_CURVE_LAT_MUL;
    }
    if (ring_dir != 0)
    {
        kp_now *= RING_LAT_MUL;
    ...
    head_term = (head_kp_now * head_preview) + (head_kd_now * preview_delta);
    if (ring_dir != 0)
    {
        head_term += (float)ring_dir * RING_HEAD_BIAS;
    }
    ...
    if (exit_phase && (ring_dir == 0) && (!s_curve_flag))
    {
    ...
    if ((s_curve_flag) && (ring_dir == 0))
    {
        speed_scale_total *= S_CURVE_SPEED_KEEP;
    }
    if (ring_dir != 0)
    {
        speed_scale_total *= RING_SPEED_KEEP;
    }
    if (low_conf)
    {
        speed_scale_total *= LOW_CONF_SPEED_KEEP;
    }
    ...
    control_debug_steer_x10 = (int16)(steer_out * 10.0f);
    control_debug_target_pulses = (int16)target_pulses;
    control_debug_left_target = (int16)left_target_pulses;
    control_debug_right_target = (int16)right_target_pulses;
    control_debug_flags = (uint8)((straight_lock ? 1 : 0) |
                                  (outside_drift ? 2 : 0) |
                                  ((ring_dir != 0) ? 4 : 0) |
                                  (s_curve_flag ? 8 : 0) |
                                  (low_conf ? 16 : 0));
