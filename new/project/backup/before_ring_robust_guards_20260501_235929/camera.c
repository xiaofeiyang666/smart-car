...
#define CAMERA_RING_FIRST_MOUTH_BLIND_FRAMES 8
#define CAMERA_RING_WAIT_ENTRY_LOST_FRAMES 12
#define CAMERA_RING_ENTRY_ARM_FRAMES     115
...
void image_deal(void)
{
    ...
    ring_detect_blocked = (ring_cross_guard || (ring_cross_lock_timer > 0) || (ring_after_exit_lock_timer > 0)) ? 1 : 0;
    ...
    if (ring_hold_state == CAMERA_RING_NONE)
    {
        ...
        {
            if ((side_open_dir != 0) && ring_curve_guard && (camera_confidence >= CAMERA_RING_MIN_CONF))
            {
        ...
        else if (ring_gate_state == CAMERA_RING_GATE_IGNORE_EXIT)
        {
            if ((side_open_dir == ring_gate_dir) &&
                ...
                (side_open_near <= CAMERA_RING_FAR_ENTRY_NEAR_MAX) &&
                ring_curve_guard && (camera_confidence >= CAMERA_RING_MIN_CONF))
            {
