...
#define CAMERA_RING_EXIT_FRAMES          30
#define CAMERA_RING_EXIT_CLEAR_SCORE_TH  2
// 出环必须接近一整圈，防止半圈或错误口退出后朝反方向跑；yaw偏小仍错出就升READY/DONE。
#define CAMERA_RING_EXIT_YAW_READY       330
#define CAMERA_RING_EXIT_YAW_DONE        380
#define CAMERA_RING_EXIT_YAW_FORCE_DONE  450
// 出环复位条件：至少保持一小段EXIT，并连续看到直道特征；提前复位会重新识别同一个口。
#define CAMERA_RING_EXIT_MIN_FRAMES      8
#define CAMERA_RING_EXIT_CLEAR_FRAMES    3
...
// 环岛只允许在稳定直道旁武装；连续反向弯/近距离并行白道会屏蔽未入环状态，避免左下大半径反向弯误触发。
#define CAMERA_RING_ARM_STRAIGHT_FRAMES  2
#define CAMERA_RING_ARM_PREVIEW_MAX      14
#define CAMERA_RING_ARM_FAR_MAX          28
#define CAMERA_RING_REVERSE_NEAR_TH      6
#define CAMERA_RING_REVERSE_FAR_TH       18
#define CAMERA_RING_CLOSE_REJECT_BLOCK_TH 8
...
#define CAMERA_RING_GATE_LOCKOUT         3
#define CAMERA_RING_GATE_IGNORE_EXIT     4
#define CAMERA_RING_FIRST_MOUTH_HITS_TH  3
#define CAMERA_RING_FIRST_MOUTH_DOM_TH   2
#define CAMERA_RING_FIRST_MOUTH_NEAR_TH  2
#define CAMERA_RING_FIRST_MOUTH_MIN_FRAMES 2
#define CAMERA_RING_FIRST_MOUTH_GAP_FRAMES 1
// 近处出口消失后先盲走几帧，不识别、不补线入环；近处仍误打方向就升，真正入口错过就降。
#define CAMERA_RING_FIRST_MOUTH_BLIND_FRAMES 5
#define CAMERA_RING_WAIT_ENTRY_LOST_FRAMES 12
#define CAMERA_RING_WAIT_ENTRY_NEAR_MAX  3
// A/B点几何约束：用连续丢边段的下端A、上端B确认口的位置，减少普通弯道和近距离并行白道误触发。
// FIRST_NEAR越大越不容易把远处假开口当第一个口；ENTRY_FAR越大越稳但入口会晚。
#define CAMERA_RING_AB_NEAR_ROW_MIN      86
#define CAMERA_RING_AB_FIRST_NEAR_HITS   3
#define CAMERA_RING_AB_ENTRY_FAR_HITS    4
#define CAMERA_RING_AB_ENTRY_NEAR_MAX    3
#define CAMERA_RING_AB_SEG_MIN           3
#define CAMERA_RING_AB_SIDE_DOM          3
#define CAMERA_RING_AB_OPP_DELTA_MAX     26
#define CAMERA_RING_AB_MID_DELTA_MAX     32
#define CAMERA_RING_ENTRY_ARM_FRAMES     65
// 出环后会再次看见环岛入口，按开源状态机思路短暂补线直行并屏蔽识别；误屏蔽下一处环岛就降LOCK_FRAMES。
#define CAMERA_RING_EXIT_LOCK_FRAMES     90
#define CAMERA_RING_POST_EXIT_BIAS       7
#define CAMERA_RING_POST_EXIT_PREVIEW_BIAS 6
// 十字/十字接圆环出现双侧开口后，普通直道旁环岛识别短暂让权，防止两套元素互相抢状态。
#define CAMERA_RING_CROSS_LOCK_FRAMES    22
// 环岛偏置：远端入口触发后就开始给方向；若进环后切太狠就减小MAX/PREVIEW，进不去就升PRE_ENTRY或PREVIEW。
#define CAMERA_RING_PRE_ENTRY_SCORE_TH   6
#define CAMERA_RING_PRE_ENTRY_MID_BIAS   46
#define CAMERA_RING_PRE_ENTRY_PREVIEW_BIAS 50
#define CAMERA_RING_PRE_ENTRY_PREVIEW_MIN 50
#define CAMERA_RING_ENTER_VIRTUAL_BIAS   58
#define CAMERA_RING_INSIDE_VIRTUAL_BIAS  50
// 出环分两段：先继续沿环转到出口，再反向轻扶回直线；直着冲出就升KEEP，回正过猛就降ALIGN。
#define CAMERA_RING_EXIT_KEEP_YAW        370
#define CAMERA_RING_EXIT_KEEP_BIAS       38
#define CAMERA_RING_EXIT_KEEP_PREVIEW    42
#define CAMERA_RING_EXIT_ALIGN_BIAS      16
#define CAMERA_RING_EXIT_ALIGN_PREVIEW   24
#define CAMERA_RING_ENTER_PREVIEW_MIN    64
#define CAMERA_RING_INSIDE_PREVIEW_MIN   54
#define CAMERA_RING_MID_BIAS_MIN         11
#define CAMERA_RING_MID_BIAS_MAX         36
#define CAMERA_RING_PREVIEW_BIAS         32
// EXIT阶段改为离开环岛侧的补线；仍被吸回环岛就升，出环冲外侧就降。
#define CAMERA_RING_EXIT_MID_BIAS        4
#define CAMERA_RING_EXIT_PREVIEW_BIAS    6
...
static uint8 ring_gate_blind_timer = 0;
static uint8 ring_exit_clear_frames = 0;
static uint8 ring_after_exit_lock_timer = 0;
static int8 ring_after_exit_lock_dir = 0;
static uint8 ring_cross_lock_timer = 0;
static uint8 ring_arm_straight_frames = 0;
static uint8 left_edge_real_list[MT9V03X_H];
static uint8 right_edge_real_list[MT9V03X_H];
...
// 环岛A/B点几何检测：在某一侧寻找连续丢边段，下端视作A点，上端视作B点。
// 返回first_ok表示近处第一个口，entry_ok表示远处真正入口；对侧边线和中线过弯太大时直接否决。
static void detect_ring_ab_side(int dir, int *total_count, int *far_count, int *near_count, uint8 *first_ok, uint8 *entry_ok)
{
    int row;
    int miss;
    int cur_len;
    ...
    *entry_ok = 0;
    ...
    for (row = CAMERA_RING_ROW_MAX; row >= CAMERA_RING_ROW_MIN; row--)
    {
        if (dir < 0)
        {
            miss = ((left_edge_real_list[row] == 0) || (left_line_list[row] <= 2)) ? 1 : 0;
        }
        else
        {
            miss = ((right_edge_real_list[row] == 0) || (right_line_list[row] >= MT9V03X_W - 3)) ? 1 : 0;
        }

        if (miss)
        {
            (*total_count)++;
            if (row <= CAMERA_RING_FAR_ROW_MAX) (*far_count)++;
            else (*near_count)++;
    ...
    near_row = clamp_i(CAMERA_RING_ROW_MAX, search_end_line + 1, MT9V03X_H - 1);
    far_row = clamp_i(CAMERA_RING_ROW_MIN + 8, search_end_line + 1, MT9V03X_H - 1);
    if (dir < 0)
    {
        opp_delta = abs_i((int)right_line_list[near_row] - (int)right_line_list[far_row]);
    }
    else
    {
        opp_delta = abs_i((int)left_line_list[near_row] - (int)left_line_list[far_row]);
    }
    mid_delta = abs_i((int)mid_line_list[near_row] - (int)mid_line_list[far_row]);

    if ((*near_count >= CAMERA_RING_AB_FIRST_NEAR_HITS) &&
        (best_len >= CAMERA_RING_AB_SEG_MIN) &&
        (best_bottom >= CAMERA_RING_AB_NEAR_ROW_MIN) &&
        (opp_delta <= CAMERA_RING_AB_OPP_DELTA_MAX) &&
        (mid_delta <= CAMERA_RING_AB_MID_DELTA_MAX))
    {
        *first_ok = 1;
    }

    if ((*far_count >= CAMERA_RING_AB_ENTRY_FAR_HITS) &&
        (*near_count <= CAMERA_RING_AB_ENTRY_NEAR_MAX) &&
        (best_len >= CAMERA_RING_AB_SEG_MIN) &&
        (best_top <= CAMERA_RING_FAR_ROW_MAX) &&
        (opp_delta <= CAMERA_RING_AB_OPP_DELTA_MAX + 8) &&
        (mid_delta <= CAMERA_RING_AB_MID_DELTA_MAX + 10))
    {
        *entry_ok = 1;
    }
}
...
void image_deal(void)
{
    ...
    int left_ab_near;
    int right_ab_near;
    int ring_ab_first_dir;
    int ring_ab_entry_dir;
    uint8 left_ab_first_ok;
    uint8 right_ab_first_ok;
    uint8 left_ab_entry_ok;
    uint8 right_ab_entry_ok;
    int ring_preview_tmp;
    int ring_far_tmp;
    uint8 ring_curve_guard;
    uint8 ring_cross_guard;
    uint8 ring_far_guard;
    uint8 ring_bend_guard;
    uint8 ring_close_guard;
    uint8 ring_arm_ready;
    uint8 ring_keep_ok;
    uint8 ring_detect_blocked;
    uint8 ring_exit_visual_clear;
    uint8 ring_exit_min_done;
    uint8 prev_low_conf;
    for (row = 0; row < MT9V03X_H; row++)
    {
        ...
        left_edge_real_list[row] = 0;
        right_edge_real_list[row] = 0;
    }
    ...
    for (row = search_start_line - 1; row > search_end_line; row -= CAMERA_ROW_STEP)
    {
        ...
        left_edge_real_list[row] = left_rebuilt ? 0 : 1;
        right_edge_real_list[row] = right_rebuilt ? 0 : 1;
        ...
        if (CAMERA_ROW_STEP > 1)
        {
            for (fill_row = row + 1; (fill_row < row + CAMERA_ROW_STEP) && (fill_row < search_start_line); fill_row++)
            {
                if (fill_row > search_end_line)
                {
                    ...
                    left_edge_real_list[fill_row] = left_edge_real_list[row];
                    right_edge_real_list[fill_row] = right_edge_real_list[row];
                }
            }
        }
    ...
    detect_ring_ab_side(-1, &left_ab_total, &left_ab_far, &left_ab_near, &left_ab_first_ok, &left_ab_entry_ok);
    detect_ring_ab_side(1, &right_ab_total, &right_ab_far, &right_ab_near, &right_ab_first_ok, &right_ab_entry_ok);

    ring_ab_first_dir = 0;
    if (left_ab_first_ok && !right_ab_first_ok && (left_ab_total >= right_ab_total + CAMERA_RING_AB_SIDE_DOM))
        ring_ab_first_dir = -1;
    else if (right_ab_first_ok && !left_ab_first_ok && (right_ab_total >= left_ab_total + CAMERA_RING_AB_SIDE_DOM))
        ring_ab_first_dir = 1;

    ring_ab_entry_dir = 0;
    if (left_ab_entry_ok && !right_ab_entry_ok && (left_ab_far >= right_ab_far + CAMERA_RING_AB_SIDE_DOM))
        ring_ab_entry_dir = -1;
    else if (right_ab_entry_ok && !left_ab_entry_ok && (right_ab_far >= left_ab_far + CAMERA_RING_AB_SIDE_DOM))
        ring_ab_entry_dir = 1;

    // 调试输出改为A/B几何检测分数：score=侧边连续丢边总量，rf=远端A/B入口量，rn=近处第一个口量。
    ring_score_tmp = (left_ab_total > right_ab_total) ? left_ab_total : right_ab_total;
    ring_far_score_tmp = (left_ab_far > right_ab_far) ? left_ab_far : right_ab_far;
    ring_near_score_tmp = (left_ab_near > right_ab_near) ? left_ab_near : right_ab_near;
    ...
    ring_bend_guard = ((((ring_preview_tmp > CAMERA_RING_REVERSE_NEAR_TH) &&
                         (ring_far_tmp < -CAMERA_RING_REVERSE_FAR_TH)) ||
                        ((ring_preview_tmp < -CAMERA_RING_REVERSE_NEAR_TH) &&
                         (ring_far_tmp > CAMERA_RING_REVERSE_FAR_TH))) ? 1 : 0);
    ring_close_guard = (close_track_reject_cnt >= CAMERA_RING_CLOSE_REJECT_BLOCK_TH) ? 1 : 0;

    if (!ring_bend_guard && !ring_close_guard &&
        (abs_i(ring_preview_tmp) <= CAMERA_RING_ARM_PREVIEW_MAX) &&
        (abs_i(ring_far_tmp) <= CAMERA_RING_ARM_FAR_MAX) &&
        (camera_confidence >= CAMERA_RING_MIN_CONF))
    {
        if (ring_arm_straight_frames < 255) ring_arm_straight_frames++;
    }
    else if (ring_arm_straight_frames > 0)
    {
        ring_arm_straight_frames--;
    }
    ring_arm_ready = (ring_arm_straight_frames >= CAMERA_RING_ARM_STRAIGHT_FRAMES) ? 1 : 0;
    ...
    if (ring_cross_guard)
    {
        ring_cross_lock_timer = CAMERA_RING_CROSS_LOCK_FRAMES;
    }
    else if (ring_cross_lock_timer > 0)
    {
        ring_cross_lock_timer--;
    }
    ...
    if (ring_after_exit_lock_timer > 0)
    {
        if (side_open_dir == ring_after_exit_lock_dir)
        {
            ring_after_exit_lock_timer = CAMERA_RING_EXIT_LOCK_FRAMES;
        }
        else
        {
            ring_after_exit_lock_timer--;
            if (ring_after_exit_lock_timer == 0)
                ring_after_exit_lock_dir = 0;
        }
    }

    ring_detect_blocked = (ring_cross_guard || (ring_cross_lock_timer > 0) || ring_close_guard ||
                           ((ring_bend_guard != 0) && (ring_gate_state != CAMERA_RING_GATE_WAIT_ENTRY)) ||
                           (ring_after_exit_lock_timer > 0)) ? 1 : 0;

    ring_raw_entry_dir = 0;
    if (!ring_detect_blocked && (camera_confidence >= CAMERA_RING_MIN_CONF))
    {
        // 入口方向只认A/B几何入口，旧的开口计数只保留做调试分数，避免普通弯道抢环岛状态。
        ring_raw_entry_dir = ring_ab_entry_dir;
    }
    ...
    if (ring_hold_state == CAMERA_RING_NONE)
    {
        ...
        else if (ring_gate_state == CAMERA_RING_GATE_PASS_EXIT)
        {
            ...
            if ((ring_ab_first_dir == ring_gate_dir) ||
                ((side_open_dir == ring_gate_dir) && (side_open_near >= CAMERA_RING_FIRST_MOUTH_NEAR_TH)))
            {
                ring_gate_lost_frames = 0;
            }
            else
            {
                ...
                {
                    ring_gate_state = CAMERA_RING_GATE_IGNORE_EXIT;
                    ring_gate_blind_timer = CAMERA_RING_FIRST_MOUTH_BLIND_FRAMES;
                    ring_gate_lost_frames = 0;
                }
            }
        }
        else if (ring_gate_state == CAMERA_RING_GATE_IGNORE_EXIT)
        {
            if ((ring_ab_entry_dir == ring_gate_dir) &&
                !ring_bend_guard && !ring_close_guard &&
                (camera_confidence >= CAMERA_RING_MIN_CONF))
            {
                // 同侧远端口已经出现，说明真正入口到了，立即放行而不是继续屏蔽。
                ...
                ring_far_guard = 1;
                if (ring_raw_entry_dir == 0)
                    ring_raw_entry_dir = ring_gate_dir;
            }
            else if ((side_open_dir == ring_gate_dir) && (side_open_near >= CAMERA_RING_ENTER_NEAR_HITS_TH))
            {
                // 仍是近处出口压在画面下方，继续盲区。
                ring_gate_blind_timer = CAMERA_RING_FIRST_MOUTH_BLIND_FRAMES;
                ring_gate_lost_frames = 0;
            }
            else if (ring_gate_blind_timer > 0)
            {
                ring_gate_blind_timer--;
            }
            else
            {
                ring_gate_state = CAMERA_RING_GATE_WAIT_ENTRY;
        ...
        {
            if ((ring_ab_entry_dir == ring_gate_dir) && !ring_bend_guard && !ring_close_guard &&
                (camera_confidence >= CAMERA_RING_MIN_CONF))
            {
                ring_gate_allow_entry = 1;
                ring_far_guard = 1;
            ...
            {
                if (ring_gate_lost_frames < 255) ring_gate_lost_frames++;
                if (ring_gate_lost_frames >= CAMERA_RING_WAIT_ENTRY_LOST_FRAMES)
                {
                    ring_gate_state = CAMERA_RING_GATE_NONE;
                    ring_gate_dir = 0;
                    ring_gate_timer = 0;
    ...
    {
        // 环岛已锁定后，入口方向不再跟随单帧识别翻转，避免环内左右互相打架。
        ring_entry_dir = ring_hold_dir;
    }
    ...
    else
    {
        ...
        {
            if (ring_hold_timer > 0) ring_hold_timer--;
            if ((control_ring_yaw_abs >= CAMERA_RING_EXIT_YAW_READY) &&
                ((ring_hold_timer == 0) ||
        ...
        else
        {
            ...
            ring_exit_visual_clear = ((side_total <= CAMERA_RING_EXIT_CLEAR_SCORE_TH) &&
                                      (side_far <= 1) && (side_near <= 1) &&
                                      (abs_i(ring_preview_tmp) <= CAMERA_RING_STRAIGHT_PREVIEW_MAX) &&
                                      (abs_i(ring_far_tmp) <= CAMERA_RING_FAR_PREVIEW_MAX) &&
                                      (camera_confidence >= 45)) ? 1 : 0;
            if (ring_exit_visual_clear)
            {
                if (ring_exit_clear_frames < 255) ring_exit_clear_frames++;
            }
            else
            {
                ring_exit_clear_frames = 0;
            ...
            ring_exit_min_done = (ring_hold_timer <= (uint8)(CAMERA_RING_EXIT_FRAMES - CAMERA_RING_EXIT_MIN_FRAMES)) ? 1 : 0;
            if (ring_exit_min_done &&
                (((control_ring_yaw_abs >= CAMERA_RING_EXIT_YAW_DONE) &&
                  (ring_exit_clear_frames >= CAMERA_RING_EXIT_CLEAR_FRAMES)) ||
                 ((control_ring_yaw_abs >= CAMERA_RING_EXIT_YAW_FORCE_DONE) &&
                  (abs_i(ring_preview_tmp) <= CAMERA_RING_FAR_PREVIEW_MAX) &&
                  (camera_confidence >= 35))))
            {
                ring_after_exit_lock_dir = ring_hold_dir;
                ring_after_exit_lock_timer = CAMERA_RING_EXIT_LOCK_FRAMES;
                ring_hold_state = CAMERA_RING_NONE;
                ring_hold_dir = 0;
                ring_hold_timer = 0;
    ...
    camera_ring_gate_state_debug = (ring_after_exit_lock_timer > 0) ? CAMERA_RING_GATE_LOCKOUT : ring_gate_state;
}
...
uint8 find_mid_line_weight(void)
{
    ...
    int ring_state_tmp;
    int ring_score_tmp;
    int ring_prepare_dir_tmp;
    int ring_preview_min_tmp;
    int ring_virtual_bias_tmp;
    int ring_virtual_preview_tmp;
    int step_limit;
    int32 delta_x8;
    int32 target_mid_x8;
    int preview_tmp;
    ...
    racing_bias = 0;
    s_transition = 0;
    ring_prepare_dir_tmp = 0;
    if ((ring_gate_state == CAMERA_RING_GATE_WAIT_ENTRY) && (ring_gate_dir != 0) &&
        (ring_after_exit_lock_timer == 0) && (ring_score_tmp >= CAMERA_RING_PRE_ENTRY_SCORE_TH))
    {
        ring_prepare_dir_tmp = (int)ring_gate_dir;
    }

    if ((ring_state_tmp != CAMERA_RING_NONE) && (ring_dir_tmp != 0))
    {
        // 环岛期间使用虚拟轨迹，暂时不追原直道中线，避免位置环被直道白区拉回。
        if (ring_state_tmp == CAMERA_RING_EXIT)
        {
            if (control_ring_yaw_abs < CAMERA_RING_EXIT_KEEP_YAW)
                ring_virtual_bias_tmp = ring_dir_tmp * CAMERA_RING_EXIT_KEEP_BIAS;
            else
                ring_virtual_bias_tmp = -ring_dir_tmp * CAMERA_RING_EXIT_ALIGN_BIAS;
        }
        else if (ring_state_tmp == CAMERA_RING_ENTER)
        {
            ring_virtual_bias_tmp = ring_dir_tmp * CAMERA_RING_ENTER_VIRTUAL_BIAS;
        }
        else
        {
            ring_virtual_bias_tmp = ring_dir_tmp * CAMERA_RING_INSIDE_VIRTUAL_BIAS;
        }
        mid_new = MID_W + ring_virtual_bias_tmp;
    }
    else if (ring_prepare_dir_tmp != 0)
    {
        // 第二个入口出现但尚未正式入环时，提前切到入环虚拟目标，避免电机速度下错过入口。
        mid_new = MID_W + ring_prepare_dir_tmp * CAMERA_RING_PRE_ENTRY_MID_BIAS;
    }
    else if ((ring_after_exit_lock_timer > 0) && (ring_after_exit_lock_dir != 0))
    {
        // 出环后补线：临时远离环岛口，避免刚离开又被入口白边吸回去。
        ring_bias = CAMERA_RING_POST_EXIT_BIAS;
        if (ring_after_exit_lock_timer < (CAMERA_RING_EXIT_LOCK_FRAMES / 2))
            ring_bias = (ring_bias + 1) / 2;
        mid_new -= (int)ring_after_exit_lock_dir * ring_bias;
    }
    ...
    else
    {
        ...
        if (((ring_state_tmp != CAMERA_RING_NONE) && (ring_dir_tmp != 0)) || (ring_prepare_dir_tmp != 0))
            step_limit = CAMERA_FINAL_MID_STEP_MAX + 8;
    ...
    if ((ring_state_tmp != CAMERA_RING_NONE) && (ring_dir_tmp != 0))
    {
        ...
        {
            if (control_ring_yaw_abs < CAMERA_RING_EXIT_KEEP_YAW)
                preview_tmp = -ring_dir_tmp * CAMERA_RING_EXIT_KEEP_PREVIEW;
            else
                preview_tmp = ring_dir_tmp * CAMERA_RING_EXIT_ALIGN_PREVIEW;
        }
        else
        {
            // 预瞄符号与位置目标配套：ring_dir=1时位置目标在右侧，舵机需要负向入环。
            ring_preview_min_tmp = (ring_state_tmp == CAMERA_RING_ENTER) ? CAMERA_RING_ENTER_PREVIEW_MIN : CAMERA_RING_INSIDE_PREVIEW_MIN;
            ring_virtual_preview_tmp = -ring_dir_tmp * ring_preview_min_tmp;
            preview_tmp = ring_virtual_preview_tmp;
        }
    }
    else if (ring_prepare_dir_tmp != 0)
    {
        preview_tmp = -ring_prepare_dir_tmp * CAMERA_RING_PRE_ENTRY_PREVIEW_MIN;
    }
    else if ((ring_after_exit_lock_timer > 0) && (ring_after_exit_lock_dir != 0))
    {
        preview_tmp -= (int)ring_after_exit_lock_dir * CAMERA_RING_POST_EXIT_PREVIEW_BIAS;
    }
