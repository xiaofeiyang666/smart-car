...
// 出环必须接近一整圈，防止半圈或错误口退出后朝反方向跑；yaw偏小仍错出就升READY。
#define CAMERA_RING_EXIT_YAW_READY       300
#define CAMERA_RING_EXIT_YAW_DONE        340
...
#define CAMERA_RING_EXIT_MID_BIAS        3
#define CAMERA_RING_EXIT_PREVIEW_BIAS    5
...
void image_deal(void)
{
    ...
    else
    {
        ...
        {
            if (ring_hold_timer > 0) ring_hold_timer--;
            if ((control_ring_yaw_abs >= CAMERA_RING_EXIT_YAW_DONE) &&
                ((ring_hold_timer == 0) ||
                 ((side_total <= CAMERA_RING_EXIT_CLEAR_SCORE_TH) &&
