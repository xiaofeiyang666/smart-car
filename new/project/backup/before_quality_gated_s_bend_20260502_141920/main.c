...
void main(void)
{
    ...
    while (1)
    {
        ...
        if (print_flag)
        {
            ...
            {
                uart_dbg_div = 0;
                sprintf(debug_str, "fps=%u,off=%d,pre=%d,far=%d,cur=%d,m=%u,conf=%u\r\n", current_fps, camera_bias_raw, camera_preview_raw, camera_preview_far_raw, camera_curve_raw, camera_route_mode, camera_confidence);
