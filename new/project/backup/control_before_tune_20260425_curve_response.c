...
#define BASE_SPEED_MATCH_GAIN            1.02f
...
// 模式切换阈值（带滞回）
#define MODE_CURVE_ON                    0.38f
#define MODE_CURVE_OFF                   0.24f
#define MODE_SHARP_ON                    0.78f
#define MODE_SHARP_OFF                   0.58f
#define CURVE_EXIT_HOLD_CYCLES           8
#define MODE_HOLD_INTENSITY              0.45f
#define MODE_CURVE_RISE_ON                0.10f
#define MODE_SHARP_RISE_ON                0.22f

// 舵机
#define SERVO_SOFT_DEADBAND_PIX          2.4f
#define SERVO_OUT_LIMIT                  20.0f
#define RIGHT_TURN_STEER_MUL             1.12f
#define LEFT_TURN_STEER_MUL              1.04f
#define STEER_KP_ADD                     0.95f
#define STEER_KD_ADD                     0.22f
#define STEER_FF_BASE_MUL                0.28f
#define STEER_FF_ADD                     1.18f
#define STEER_DPREVIEW_FF_K              0.55f
...
#define STEER_STEP_SHARP                 6.40f
#define STEER_REVERSE_DPREVIEW_TH        1.8f
#define STEER_REVERSE_STEP_MUL           1.35f

// 速度
#define SPEED_DROP_MAX                   0.44f
#define SPEED_CURVE_K                    1.05f
#define SPEED_STEER_K                    0.48f
#define SPEED_CURVE_MIN                  0.40f
#define SPEED_STEER_MIN                  0.52f
#define SPEED_SCALE_MIN                  0.34f
#define SPEED_SCALE_MAX                  0.98f
#define HARD_BRAKE_PREVIEW_TH            6.2f
#define HARD_BRAKE_DPREVIEW_TH           0.42f
#define HARD_BRAKE_SCORE_RISE_TH         0.16f
#define HARD_BRAKE_KEEP                  0.58f
#define HARD_BRAKE_HOLD_CYCLES           10

// 目标速度斜坡（20ms 周期）
#define TARGET_STEP_UP_STRAIGHT          3.5f
#define TARGET_STEP_UP_SHARP             0.70f
...
void control_loop(void)
{
    ...
    if (curve_score_rise < 0.0f) curve_score_rise = 0.0f;
    ...
    if (road_mode == ROAD_STRAIGHT)
    {
        if (curve_hold_cnt > 0) curve_hold_cnt--;
    }
    else
    {
    ...
    // 模式强度（0/0.55/1.0），直道但处于出弯保持时不立即抬速
    if (road_mode == ROAD_STRAIGHT)
    {
        if (curve_hold_cnt > 0) mode_intensity = MODE_HOLD_INTENSITY;
        else mode_intensity = 0.0f;
    }
    else if (road_mode == ROAD_CURVE)
    {
        mode_intensity = 0.55f;
    }
    else
    {
        mode_intensity = 1.0f;
    ...
    ff_term = kff_now * (preview_filtered + STEER_DPREVIEW_FF_K * preview_delta);
