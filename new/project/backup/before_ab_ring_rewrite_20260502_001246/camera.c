...
void image_deal(void)
{
    ...
    if (!ring_detect_blocked && (camera_confidence >= CAMERA_RING_MIN_CONF) &&
        (ring_curve_guard || (ring_gate_state == CAMERA_RING_GATE_WAIT_ENTRY)))
    {
    ...
    if (ring_hold_state == CAMERA_RING_NONE)
    {
        ...
        {
            if ((side_open_dir != 0) && ring_arm_ready && ring_curve_guard &&
                (side_open_near >= CAMERA_RING_FIRST_MOUTH_NEAR_TH) &&
                (camera_confidence >= CAMERA_RING_MIN_CONF))
            {
        ...
        {
            if ((side_open_dir == ring_gate_dir) &&
                (side_open_far >= CAMERA_RING_FAR_ENTRY_HITS_TH) &&
                (side_open_near <= CAMERA_RING_WAIT_ENTRY_NEAR_MAX) &&
        ...
        {
            if ((side_open_dir == ring_gate_dir) && !ring_bend_guard && !ring_close_guard &&
                (side_open_far >= CAMERA_RING_FAR_ENTRY_HITS_TH) &&
                (side_open_near <= CAMERA_RING_WAIT_ENTRY_NEAR_MAX) &&
                (camera_confidence >= CAMERA_RING_MIN_CONF))
            {
            ...
            if (ring_gate_allow_entry && (ring_raw_entry_dir == 0) &&
                (side_open_total >= CAMERA_RING_FULL_ENTRY_HITS_TH) &&
                (side_open_far >= CAMERA_RING_FULL_ENTRY_FAR_TH))
            {
                ring_raw_entry_dir = ring_gate_dir;
            }
