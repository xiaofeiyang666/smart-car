...
#define FAR_PREVIEW_HEAD_GAIN            0.20f
...
#define S_TRANSITION_FAR_TH              7.0f
#define S_TRANSITION_PREVIEW_MAX         42.0f
#define S_TRANSITION_CONF_MIN            12
#define S_TRANSITION_STRONG_FAR_TH       24.0f
#define S_TRANSITION_STRONG_PREVIEW_MAX  14.0f
#define S_TRANSITION_HEAD_GAIN           0.38f
#define S_TRANSITION_FF_LIMIT_ADD        1.7f
#define S_TRANSITION_STEP_MUL            1.25f
#define S_TRANSITION_SPEED_KEEP          0.68f
#define S_TRANSITION_LOCK_BREAK_FAR_TH   16.0f

// 远前瞻主导：far已经明显看到下一个大弯时，暂时降低近端off/外漂对舵量的反向拉扯。
#define FAR_DOMINANT_TH                  30.0f
#define FAR_DOMINANT_PREVIEW_TH          18.0f
#define FAR_DOMINANT_HEAD_GAIN           0.18f
#define FAR_DOMINANT_FF_LIMIT_ADD        1.4f
#define FAR_DOMINANT_LAT_MUL             0.45f
#define FAR_DOMINANT_LAT_D_MUL           0.55f
#define FAR_DOMINANT_GYRO_MUL            0.55f
#define FAR_DOMINANT_STEP_MUL            1.22f
#define FAR_DOMINANT_SPEED_KEEP          0.72f
...
#define OUTSIDE_FF_SUPPRESS              0.72f
#define OUTSIDE_PULL_K                   0.24f
#define OUTSIDE_STEP_MUL                 1.18f
#define OUTSIDE_SPEED_KEEP               0.82f
...
void control_loop(void)
{
    ...
    straight_lock_raw = 0;
    if ((abs_preview <= STRAIGHT_LOCK_PREVIEW_TH) &&
    ...
    s_transition_line = 0;
    if (((straight_lock == 0) || (abs_far_preview >= S_TRANSITION_LOCK_BREAK_FAR_TH)) &&
        ...
        ((((preview_filtered > 1.0f) && (preview_far_filtered < -S_TRANSITION_FAR_TH)) ||
          ((preview_filtered < -1.0f) && (preview_far_filtered > S_TRANSITION_FAR_TH)) ||
          ((abs_preview <= 6.0f) && (abs_far_preview >= S_TRANSITION_FAR_TH)) ||
    ...
    if (!s_transition_line &&
        (abs_far_preview >= FAR_DOMINANT_TH) &&
        (abs_preview >= FAR_DOMINANT_PREVIEW_TH) &&
        same_curve &&
        (camera_confidence >= 18))
    {
        far_dominant_line = 1;
    }

    if (s_transition_line || far_dominant_line)
    {
        straight_lock = 0;
        straight_lock_cnt = 0;
        small_curve_line = 0;
    ...
    if (far_dominant_line)
    {
        kp_mul *= FAR_DOMINANT_LAT_MUL;
        kd_mul *= FAR_DOMINANT_GYRO_MUL;
        step_base *= FAR_DOMINANT_STEP_MUL;
    }
    ...
    if (far_dominant_line)
    {
        lat_d_now *= FAR_DOMINANT_LAT_D_MUL;
    }
    ...
    if (far_dominant_line)
    {
        head_term += head_kp_now * FAR_DOMINANT_HEAD_GAIN * preview_far_filtered;
    }
    ...
    if (far_dominant_line)
    {
        ff_limit += FAR_DOMINANT_FF_LIMIT_ADD;
    }
    if (!s_transition_line && (abs_err <= STEER_FF_SUPPRESS_ERR_TH) && (abs_preview <= STEER_FF_SUPPRESS_PREVIEW_TH))
    {
    ...
    if (outside_drift && !far_dominant_line && !s_transition_line)
    {
    ...
    if (far_dominant_line)
    {
        steer_step_limit *= FAR_DOMINANT_STEP_MUL;
    }
    if (straight_lock)
    {
    ...
    control_debug_s_transition = s_transition_line ? 1 : (far_dominant_line ? 2 : 0);
    ...
    if (far_dominant_line)
    {
        speed_scale_total *= FAR_DOMINANT_SPEED_KEEP;
    }
