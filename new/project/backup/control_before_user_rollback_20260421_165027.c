...
#define OFFSET_LPF_ALPHA_STRAIGHT       0.35f            // 直道偏差低通；直线修正慢增大，直线抖动大减小
...
// ------------------------- 舵机基础参数 -------------------------
#define SERVO_DEADBAND_PIX              3                // 偏差死区；直线抖动大增大，直线不修正减小
#define SERVO_OUT_LIMIT                 23.0f            // 软件舵角限幅；转不过弯增大，机械干涉风险减小
#define RIGHT_TURN_STEER_MUL          1.16f            // 右转打角补偿倍数；右大弯打角不足则增大，右转过冲则减小
#define LEFT_TURN_STEER_MUL           1.00f            // 左转打角补偿倍数；通常保持 1.00

#define SERVO_RATE_STRAIGHT             1.20f            // 直道每周期最大打角变化；直线修正慢增大，直线抽动减小
#define SERVO_RATE_ENTRY                4.60f            // 入弯打角速率；入弯晚增大，入弯过猛减小
#define SERVO_RATE_APEX                 5.80f            // 弯中打角速率；大弯贴外线增大，弯中抖动减小
#define SERVO_RATE_EXIT                 3.40f            // 出弯打角速率；S弯二弯响应慢增大
#define SERVO_RATE_RECOVERY             6.00f            // 救车打角速率；冲线救不回增大，抽动过猛减小

// 状态增益调度（在 servo_kp / servo_kd / servo_kff 基础上乘以下列倍数）
#define STEER_GAIN_STRAIGHT_MUL         1.00f            // 直道比例倍数；直道抖动大减小
#define STEER_GAIN_ENTRY_MUL            1.25f            // 入弯比例倍数；入弯晚增大
#define STEER_GAIN_APEX_MUL             1.35f            // 弯中比例倍数；贴外线增大，摆动大减小
#define STEER_GAIN_EXIT_MUL             1.10f            // 出弯比例倍数；出弯外抛增大，出弯抖动减小
#define STEER_GAIN_RECOVERY_MUL         1.20f            // 救车比例倍数；救车不够增大

#define STEER_GYRO_STRAIGHT_MUL         1.12f            // 直道陀螺阻尼倍数；直线摆动大增大
...
#define STEER_FF_STRAIGHT_MUL           0.85f            // 直道前馈倍数；直道误打角可减小
#define STEER_FF_ENTRY_MUL              1.15f            // 入弯前馈倍数；入弯慢可增大
#define STEER_FF_APEX_MUL               1.35f            // 弯中前馈倍数；大弯贴外线可增大
#define STEER_FF_EXIT_MUL               1.05f            // 出弯前馈倍数；出弯抖动可减小
#define STEER_FF_RECOVERY_MUL           1.10f            // 救车前馈倍数
#define PREVIEW_FF_LIMIT                10.5f            // 前馈限幅；弯道不够转增大，转向冲击大减小

#define REVERSE_TURN_PREVIEW_TH         3.2f             // S弯换向判定阈值；二弯进不去可减小，误触发可增大
#define REVERSE_TURN_HOLD_CYCLES        14               // S弯换向辅助持续周期；换向不及时可增大
#define REVERSE_TURN_RATE_MUL           1.55f            // S弯换向时打角速率倍数；换向慢增大，抽动大减小
#define REVERSE_TURN_FF_MUL             1.35f            // S弯换向时前馈倍数；二弯贴外线增大，过冲减小
#define REVERSE_TURN_SPEED_KEEP         0.70f            // S弯换向短时速度保持；飞出减小，掉速过多增大
...
#define STATE_ENTRY_DPREVIEW_TH         0.20f            // 入弯变化率阈值；长直冲弯减小，误触发增大
#define STATE_APEX_PREVIEW_TH           6.2f             // 弯心判定前瞻阈值；切到弯中心太晚减小

#define STATE_EXIT_PREVIEW_TH           4.2f             // 出弯判定前瞻阈值；出弯慢增大，过早出弯减小
#define STATE_EXIT_OFFSET_TH            3.6f             // 出弯判定偏差阈值；出弯回正慢增大，出弯晃动减小
...
#define LONG_STRAIGHT_BRAKE_CYCLES      16               // 入弯额外刹车持续周期；冲弯严重增大
#define LONG_STRAIGHT_ENTRY_KEEP        0.55f            // 长直入弯额外速度保持；冲弯减小，掉速多增大
#define HARD_ENTRY_PREVIEW_RAW_TH       3.8f             // 直线末端硬入弯前瞻阈值；冲弯减小，误触发增大
#define HARD_ENTRY_DPREVIEW_RAW_TH      1.00f            // 直线末端硬入弯变化率阈值；冲弯减小，误触发增大
#define HARD_ENTRY_BRAKE_CYCLES         14               // 硬入弯重刹持续周期；冲弯严重增大
#define HARD_ENTRY_BRAKE_KEEP           0.48f            // 硬入弯重刹速度保持；冲弯减小，掉速大增大

// ------------------------- 速度规划 -------------------------
#define SPEED_KEEP_ENTRY                0.58f            // 入弯状态速度保持；入弯冲出减小
#define SPEED_KEEP_APEX_BASE            0.50f            // 弯中状态基础速度保持；弯中不稳减小
#define SPEED_KEEP_EXIT                 0.72f            // 出弯状态速度保持；连续弯飞出减小
#define SPEED_KEEP_RECOVERY             0.60f            // 救车状态速度保持；救车不住减小

#define SPEED_CURVE_PREVIEW_SCALE       7.0f             // 前瞻归一化尺度；弯道减速太晚减小，太早增大
#define SPEED_CURVE_K                   1.80f            // 曲率降速强度；弯道过快不稳增大
#define SPEED_CURVE_MIN                 0.42f            // 曲率降速下限；弯中太慢增大，冲弯减小

// 目标速度斜坡（抑制速度突变导致车身不稳）
#define TARGET_STEP_UP_STRAIGHT         8.0f             // 直道每周期最大升速脉冲；突加速减小
#define TARGET_STEP_UP_TURN             4.0f             // 弯道每周期最大升速脉冲；弯中加速明显减小
#define TARGET_STEP_DOWN                110.0f           // 每周期最大降速脉冲；减速不及时增大
...
float servo_kd = 0.09f;                                   // 陀螺阻尼；摆动大增大，转向迟滞减小
float servo_kff = 0.42f;                                  // 前瞻前馈；贴外线增大，弯前过冲减小
