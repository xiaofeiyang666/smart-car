...
static const control_mode_param_t control_mode_params[MODE_PARAM_COUNT] =
{
    ...
    // LOW_CONF：视觉不可靠时保守相信上一趋势，限舵、限速、限差速。
    {0.72f, 1.42f, 0.32f, 1.28f, 0.32f, 2.20f, 0.62f, 0.55f}
};
...
void control_loop(void)
{
    ...
    if (camera_quality < 25) mode = CAMERA_TRACK_LOW_CONF;
    ...
    if (mode == CAMERA_TRACK_LOW_CONF) bias_alpha = 0.22f;
    ...
    if (mode == CAMERA_TRACK_LOW_CONF && steer_alpha > 0.38f) steer_alpha = 0.38f;
