...
// 串口调试开关：0=关闭（提升性能），1=开启（输出视觉和控制状态）
...
void main(void)
{
    ...
    while (1)
    {
        ...
        if (print_flag)
        {
            ...
            {
                uart_dbg_div = 0;
                sprintf(debug_str, "fps=%u,off=%d,pre=%d,slp=%d,cur=%d,conf=%u,q=%u,m=%u,sc=%d,st=%d,gz=%d\r\n", current_fps, camera_bias_raw, camera_preview_raw, camera_slope_raw, camera_curve_raw, camera_confidence, camera_quality, control_debug_mode, control_debug_curve_x100, control_debug_steer_out, control_debug_gyro_z);
