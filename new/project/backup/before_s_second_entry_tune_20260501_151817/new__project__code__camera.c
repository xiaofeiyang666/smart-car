...
#define CAMERA_RING_CONFIRM_FRAMES       3
...
#define CAMERA_RING_FAR_PREVIEW_MAX      48
...
// 赛车线偏置：只做几像素的外-内-外趋势，不允许把S弯当直线抄白区；切弯过多就降，弯道太保守可升到5~6。
#define CAMERA_RACING_BIAS_MAX           4
// S弯二次入弯：far已经看到第二个弯而near还在回正时，给小偏置防止直线冲出。误把直道当S弯就升FAR_TH或降BIAS。
#define CAMERA_S_TRANS_FAR_TH            16
#define CAMERA_S_TRANS_NEAR_MAX          20
#define CAMERA_S_TRANS_CONF_MIN          15
#define CAMERA_S_TRANS_BIAS_MAX          8
#define CAMERA_S_TRANS_PREVIEW_BIAS      10
...
void image_deal(void)
{
    ...
    ring_entry_dir = 0;
    if ((camera_confidence >= CAMERA_RING_MIN_CONF) && ring_curve_guard)
    {
    ...
    if (ring_hold_state == CAMERA_RING_NONE)
    {
        if (ring_entry_dir != 0)
        {
            ...
            {
                if (ring_candidate_frames < 255) ring_candidate_frames++;
            }
            else
            {
                ring_candidate_dir = (int8)ring_entry_dir;
                ring_candidate_frames = 1;
            }
    ...
    else
    {
        ...
        else if ((ring_score_tmp >= CAMERA_RING_KEEP_SCORE_TH) && (camera_confidence >= 20))
        {
            ring_keep_ok = 1;
        }
...
uint8 find_mid_line_weight(void)
{
    ...
    else if ((camera_confidence >= 45) &&
             (abs_preview >= 6) &&
             (abs_preview <= 26) &&
             (((preview_mid_tmp > 0) && (preview_far_tmp > 0)) ||
              ((preview_mid_tmp < 0) && (preview_far_tmp < 0))))
    {
        // 赛车线近似：只在趋势稳定时给几像素偏置，避免大舵角反复修正。
        racing_bias = preview_far_tmp / 8;
        racing_bias = clamp_i(racing_bias, -CAMERA_RACING_BIAS_MAX, CAMERA_RACING_BIAS_MAX);
        mid_new += racing_bias;
    }
