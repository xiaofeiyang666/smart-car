...
#define CAMERA_RING_PRE_ENTRY_SCORE_TH   1
...
uint8 find_mid_line_weight(void)
{
    ...
    ring_ignore_exit_tmp = ((ring_state_tmp == CAMERA_RING_NONE) &&
                            ((ring_gate_state == CAMERA_RING_GATE_PASS_EXIT) ||
                             (ring_gate_state == CAMERA_RING_GATE_IGNORE_EXIT))) ? 1 : 0;
    ...
    else if (ring_ignore_exit_tmp)
    {
        // 第一口是近处出口，只保持直行，不允许原直线边缘把舵机拉一下。
        mid_new = MID_W;
    }
    ...
    else
    {
        ...
        if (((ring_state_tmp != CAMERA_RING_NONE) && (ring_dir_tmp != 0)) || (ring_prepare_dir_tmp != 0) || ring_ignore_exit_tmp)
