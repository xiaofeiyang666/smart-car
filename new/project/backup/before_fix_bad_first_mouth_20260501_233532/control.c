...
#define S_TRANSITION_FAR_TH              9.0f
#define S_TRANSITION_PREVIEW_MAX         28.0f
#define S_TRANSITION_CONF_MIN            15
#define S_TRANSITION_STRONG_FAR_TH       24.0f
#define S_TRANSITION_STRONG_PREVIEW_MAX  14.0f
#define S_TRANSITION_HEAD_GAIN           0.30f
#define S_TRANSITION_FF_LIMIT_ADD        1.1f
#define S_TRANSITION_STEP_MUL            1.14f
#define S_TRANSITION_SPEED_KEEP          0.74f
...
#define CONTROL_RING_GATE_PASS_EXIT      1
#define CONTROL_RING_GATE_WAIT_ENTRY     2
#define CONTROL_RING_GATE_IGNORE_EXIT    4
// 环岛控制：环岛进不去先确认camera的ring/rs是否触发；已触发但舵角不够再升FF_LIMIT_ADD或相机RING_PREVIEW_BIAS。
#define RING_FF_LIMIT_ADD                2.6f
#define RING_PREPARE_FF_LIMIT_ADD        1.2f
#define RING_STEP_MUL                    1.75f
#define RING_PREPARE_STEP_MUL            1.35f
#define RING_SPEED_KEEP_ENTER            0.42f
#define RING_SPEED_KEEP_INSIDE           0.54f
#define RING_SPEED_KEEP_EXIT             0.52f
#define RING_DIFF_MUL                    0.88f
#define RING_EXIT_HEAD_KEEP              0.98f
#define RING_PREPARE_SPEED_KEEP          0.42f
...
volatile int16 control_ring_yaw_abs = 0;
...
void control_loop(void)
{
    ...
    static float ring_yaw_integral = 0.0f;
    ...
    if (!lpf_init)
    {
        ...
        ring_yaw_integral = 0.0f;
        control_ring_yaw_abs = 0;
        err_last = 0.0f;
        errd_filtered = 0.0f;
    ...
    ring_prepare = ((!ring_active) &&
                    ((camera_ring_gate_state_debug == CONTROL_RING_GATE_PASS_EXIT) ||
                     (camera_ring_gate_state_debug == CONTROL_RING_GATE_IGNORE_EXIT) ||
                     (camera_ring_gate_state_debug == CONTROL_RING_GATE_WAIT_ENTRY))) ? 1 : 0;
    ...
    if (ring_active)
    {
        ring_yaw_integral += abs_f(gyro_z_filtered) * 0.020f;
        if (ring_yaw_integral > 999.0f) ring_yaw_integral = 999.0f;
        control_ring_yaw_abs = (int16)(ring_yaw_integral + 0.5f);
    }
    else
    {
        ring_yaw_integral = 0.0f;
    ...
    small_curve_line = 0;
    if (!ring_active && !ring_prepare && !straight_lock && same_curve &&
    ...
    s_transition_line = 0;
    if (!ring_active && !ring_prepare && ((straight_lock == 0) || (abs_far_preview >= S_TRANSITION_LOCK_BREAK_FAR_TH)) &&
    ...
    {
        ff_limit += RING_PREPARE_FF_LIMIT_ADD;
    }
    if (s_transition_line)
    {
    ...
    {
        steer_step_limit *= RING_PREPARE_STEP_MUL;
    }
    ...
    if (ring_prepare)
    {
        speed_scale_total *= RING_PREPARE_SPEED_KEEP;
    }
