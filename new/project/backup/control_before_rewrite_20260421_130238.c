...
// ------------------------- 转向参数 -------------------------
#define SERVO_DEADBAND_PIX          3       // 像素死区：抑制高速直线微抖
#define SERVO_SMALL_ERR_TH          5       // 小误差阈值
#define SERVO_MID_ERR_TH            12      // 中误差阈值

#define SERVO_KP_SMALL_MUL          1.15f   // 小误差区比例增益放大倍数（抑抖优先）
...
#define SERVO_STEP_LIMIT_SMALL      1.40f    // 每20ms小误差区最大打角变化
...
#define PREVIEW_FF_ENABLE_TH        3.4f             // 前瞻前馈启动阈值（更早介入）
...
#define CORNER_STEER_BOOST_PREVIEW_TH   7.8f     // 入弯时提高打角灵敏度阈值（更早）
#define CORNER_STEER_BOOST_MUL          1.28f    // 入弯打角增强系数
#define CORNER_STEP_LIMIT_MUL           1.60f    // 入弯打角变化上限增强系数
#define CORNER_RESCUE_PREVIEW_TH         12.4f    // 急弯兜底判定阈值（速度15更早保护）
#define CORNER_RESCUE_STEER_TH           9.0f     // 大舵角兜底阈值
#define CORNER_RESCUE_SPEED_KEEP         0.80f    // 急弯兜底速度保持（速度15保护）

// ------------------------- 电子差速参数 -------------------------
#define EDIFF_K_SMALL                   0.70f    // 小舵角差速系数（回落，避免后轮主导）
#define EDIFF_K_MID                     1.10f    // 中舵角差速系数（回落，避免弯中卡滞）
#define EDIFF_K_LARGE                   1.60f    // 大舵角差速系数（回落，避免强制甩尾）
#define EDIFF_PREVIEW_BOOST_TH          11.0f    // 大曲率增强阈值（更早触发）
#define EDIFF_PREVIEW_BOOST_MUL         1.45f    // 大曲率差速增强倍数（降低激进度）
#define EDIFF_REVERSE_BOOST_MUL         1.20f    // S弯反向切换时差速增强倍数（降低激进度）
#define EDIFF_MAX                        3.00f    // 差速系数上限（收紧，防止后轮过强）
#define EDIFF_GLOBAL_GAIN               1.08f    // 后轮差速全局增强倍数（速度15保守）
#define EDIFF_OUTSIDE_DIFF_MUL          1.06f    // 外漂时差速附加倍数（轻量介入）
...
float servo_kd = 0.07f;  // 陀螺仪阻尼系数（减小转向迟滞）
