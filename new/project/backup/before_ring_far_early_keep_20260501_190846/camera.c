...
// 远端入口门槛：行号小的一侧更远。近处口只做候选，必须远端也开口才允许入环。
#define CAMERA_RING_FAR_ROW_MAX          84
#define CAMERA_RING_FAR_OPEN_HITS_TH     3
#define CAMERA_RING_FAR_NEAR_ALLOW_GAP   1
...
// 强环岛入口：直道旁开口很满但conf被近邻道路保护压低时使用。误进环岛就升CONF/ SCORE或降PREVIEW/FAR上限。
...
// 环岛偏置：进不去环岛时先小幅增大MID_BIAS_MAX/PREVIEW_BIAS；若进环岛后摆动或切太狠就减小。
...
void image_deal(void)
{
    ...
    left_entry_ok = ((left_open_hits >= CAMERA_RING_OPEN_HITS_TH) &&
                     (left_open_far_hits >= CAMERA_RING_FAR_OPEN_HITS_TH) &&
                     (left_open_far_hits + CAMERA_RING_FAR_NEAR_ALLOW_GAP >= left_open_near_hits) &&
                     (left_open_hits >= right_open_hits + CAMERA_RING_DOMINATE_TH)) ? 1 : 0;
    right_entry_ok = ((right_open_hits >= CAMERA_RING_OPEN_HITS_TH) &&
                      (right_open_far_hits >= CAMERA_RING_FAR_OPEN_HITS_TH) &&
                      (right_open_far_hits + CAMERA_RING_FAR_NEAR_ALLOW_GAP >= right_open_near_hits) &&
                      (right_open_hits >= left_open_hits + CAMERA_RING_DOMINATE_TH)) ? 1 : 0;
    ...
    ring_strong_guard = ((ring_score_tmp >= CAMERA_RING_STRONG_SCORE_TH) &&
                         ...
                         (abs_i(ring_far_tmp) <= CAMERA_RING_STRONG_FAR_MAX) &&
                         (left_entry_ok || right_entry_ok)) ? 1 : 0;
    ...
    camera_ring_far_score = (uint8)clamp_i((left_open_far_hits > right_open_far_hits) ? left_open_far_hits : right_open_far_hits, 0, 100);
    camera_ring_near_score = (uint8)clamp_i((left_open_near_hits > right_open_near_hits) ? left_open_near_hits : right_open_near_hits, 0, 100);
