...
#define CAMERA_RING_PRE_ENTRY_MID_BIAS   16
#define CAMERA_RING_PRE_ENTRY_PREVIEW_BIAS 26
#define CAMERA_RING_PRE_ENTRY_PREVIEW_MIN 32
#define CAMERA_RING_ENTER_PREVIEW_MIN    46
#define CAMERA_RING_INSIDE_PREVIEW_MIN   38
...
uint8 find_mid_line_weight(void)
{
    ...
    if ((ring_state_tmp != CAMERA_RING_NONE) && (ring_dir_tmp != 0))
    {
        ...
        {
            ring_bias = CAMERA_RING_MID_BIAS_MIN + 7 + ring_score_tmp / 2;
        }
        else if (ring_state_tmp == CAMERA_RING_INSIDE)
        {
            ring_bias = CAMERA_RING_MID_BIAS_MIN + 9 + ring_score_tmp / 3;
        }
        else
        {
            ring_bias = CAMERA_RING_EXIT_MID_BIAS;
        }
        if (ring_state_tmp == CAMERA_RING_EXIT)
        {
            ring_bias = clamp_i(ring_bias, 0, CAMERA_RING_MID_BIAS_MAX);
    ...
    {
        // 第二个入口出现但尚未正式入环时，提前向环岛内侧补线，避免电机速度下错过入口。
        mid_new += ring_prepare_dir_tmp * CAMERA_RING_PRE_ENTRY_MID_BIAS;
    }
    ...
    if ((ring_state_tmp != CAMERA_RING_NONE) && (ring_dir_tmp != 0))
    {
        ...
        else
        {
            ...
            if ((ring_dir_tmp > 0) && (preview_tmp < ring_preview_min_tmp))
                preview_tmp = ring_preview_min_tmp;
            else if ((ring_dir_tmp < 0) && (preview_tmp > -ring_preview_min_tmp))
                preview_tmp = -ring_preview_min_tmp;
        }
    }
    else if (ring_prepare_dir_tmp != 0)
    {
        preview_tmp += ring_prepare_dir_tmp * CAMERA_RING_PRE_ENTRY_PREVIEW_BIAS;
        if ((ring_prepare_dir_tmp > 0) && (preview_tmp < CAMERA_RING_PRE_ENTRY_PREVIEW_MIN))
            preview_tmp = CAMERA_RING_PRE_ENTRY_PREVIEW_MIN;
        else if ((ring_prepare_dir_tmp < 0) && (preview_tmp > -CAMERA_RING_PRE_ENTRY_PREVIEW_MIN))
            preview_tmp = -CAMERA_RING_PRE_ENTRY_PREVIEW_MIN;
    }
    else if ((ring_after_exit_lock_timer > 0) && (ring_after_exit_lock_dir != 0))
    {
