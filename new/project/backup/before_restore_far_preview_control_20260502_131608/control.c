...
#define KP_MUL_MID                       0.94f
#define KP_MUL_LARGE                     1.24f

#define KD_MUL_SMALL                     1.26f
#define KD_MUL_MID                       1.00f
#define KD_MUL_LARGE                     0.82f

#define STEP_SMALL                       1.10f
#define STEP_MID                         3.60f
#define STEP_LARGE                       8.20f
#define STEP_MIN                         0.60f

#define STEP_REVERSE_DPV_TH              1.00f
#define STEP_REVERSE_MUL                 1.35f
...
#define PREVIEW_LPF_ALPHA                0.60f
#define GYRO_LPF_ALPHA                   0.30f
#define ERRD_LPF_ALPHA                   0.40f
#define STEER_RAW_ALPHA_STRAIGHT         0.22f
#define STEER_RAW_ALPHA_CURVE            0.76f

#define STEER_FF_DPREVIEW_K              0.26f
#define STEER_FF_LIMIT_BASE              2.4f
#define STEER_FF_LIMIT_CURVE_ADD         2.2f
...
#define DUAL_PD_LAT_D_CURVE_ADD          0.18f  // 弯道时横向D增强
#define DUAL_PD_HEAD_P_CURVE_ADD         0.48f  // 弯道时前瞻P增强
#define DUAL_PD_HEAD_D_GAIN              0.90f  // 前瞻D权重（控制S弯切换速度）
#define DUAL_PD_GYRO_D_CURVE_KEEP        0.58f  // 弯中保留的陀螺阻尼比例，越小弯中越敢持续打方向
#define DUAL_PD_EXIT_HEAD_KEEP           0.75f  // 出弯时降低前瞻环，防止回直线抖动
#define LOW_CONF_CTRL_TH                 45     // 低于此置信度，控制层不再快速追随视觉跳变
#define LOW_CONF_HARD_TH                 20
#define LOW_CONF_ALPHA_MAX               0.46f
#define LOW_CONF_HARD_ALPHA_MAX          0.34f
#define LOW_CONF_STEP_MAX                3.20f
#define LOW_CONF_HARD_STEP_MAX           1.80f
#define LOW_CONF_REVERSE_BLEND           0.25f  // 低置信反向打舵时，只采纳一小部分新方向
...
#define OUTSIDE_FF_SUPPRESS              0.68f
#define OUTSIDE_PULL_K                   0.32f
#define OUTSIDE_STEP_MUL                 1.18f
#define OUTSIDE_SPEED_KEEP               0.78f
...
#define DIFF_GAIN_BASE                   0.94f
#define DIFF_GAIN_CURVE_ADD              0.38f
#define DIFF_RIGHT_MUL                   1.16f
#define DIFF_LEFT_MUL                    1.00f
#define DIFF_OUTSIDE_MUL                 1.25f

#define DIFF_FILTER_ALPHA                0.22f
#define DIFF_STEP_BASE                   12.0f
#define DIFF_STEP_CURVE_ADD              8.0f

#define DIFF_CAP_RATIO_BASE              0.20f
#define DIFF_CAP_RATIO_CURVE_ADD         0.10f
...
volatile int16 control_debug_speed_x100 = 0;
...
void control_loop(void)
{
    ...
    static int8 reliable_steer_dir = 0;
    ...
    uint8 outside_drift;
    uint8 outside_raw;
    uint8 steer_saturated;
    uint8 low_conf_ctrl;
    uint8 hard_low_conf_ctrl;
    uint8 entry_phase;
    uint8 exit_phase;
    ...
    low_conf_ctrl = (camera_confidence < LOW_CONF_CTRL_TH) ? 1 : 0;
    hard_low_conf_ctrl = (camera_confidence < LOW_CONF_HARD_TH) ? 1 : 0;
    if (camera_confidence < 55)
    {
        curve_intensity = clamp_f(curve_intensity + 0.10f, 0.0f, 1.0f);
    }
    ...
    gyro_keep = 1.0f - (1.0f - DUAL_PD_GYRO_D_CURVE_KEEP) * curve_intensity;
    if (gyro_keep < DUAL_PD_GYRO_D_CURVE_KEEP) gyro_keep = DUAL_PD_GYRO_D_CURVE_KEEP;
    kd_now = servo_kd * gyro_keep * kd_mul;
    if (camera_confidence < 55) kd_now *= 0.55f;
    ...
    if (low_conf_ctrl && (abs_f(steer_out_last) >= 2.0f) &&
        ((steer_out_last > 0.0f && steer_raw < 0.0f) || (steer_out_last < 0.0f && steer_raw > 0.0f)))
    {
        steer_raw = steer_out_last * (1.0f - LOW_CONF_REVERSE_BLEND) + steer_raw * LOW_CONF_REVERSE_BLEND;
    }
    if (low_conf_ctrl && (reliable_steer_dir != 0) &&
        (((reliable_steer_dir > 0) && (steer_raw < -2.0f)) || ((reliable_steer_dir < 0) && (steer_raw > 2.0f))) &&
        !((abs_err >= 28.0f) && (abs_preview >= 26.0f)))
    {
        steer_raw *= LOW_CONF_REVERSE_BLEND;
    }
    ...
    if (low_conf_ctrl && (steer_alpha > LOW_CONF_ALPHA_MAX)) steer_alpha = LOW_CONF_ALPHA_MAX;
    if (hard_low_conf_ctrl && (steer_alpha > LOW_CONF_HARD_ALPHA_MAX)) steer_alpha = LOW_CONF_HARD_ALPHA_MAX;
    ...
    if (low_conf_ctrl && (steer_step_limit > LOW_CONF_STEP_MAX)) steer_step_limit = LOW_CONF_STEP_MAX;
    if (hard_low_conf_ctrl && (steer_step_limit > LOW_CONF_HARD_STEP_MAX)) steer_step_limit = LOW_CONF_HARD_STEP_MAX;
    ...
    steer_saturated = (abs_steer >= (steer_limit_now * 0.88f)) ? 1 : 0;
    ...
    if (steer_saturated)
    {
        speed_scale_total *= 0.78f;
    }
    if ((conf_scale < 0.30f) && (curve_intensity >= 0.75f))
    {
        speed_scale_total *= 0.72f;
    }
    ...
    control_debug_speed_x100 = (int16)(speed_scale_total * 100.0f + 0.5f);
    if ((camera_confidence >= 55) && (abs_steer >= 3.0f))
    {
        reliable_steer_dir = (steer_out >= 0.0f) ? 1 : -1;
    }
