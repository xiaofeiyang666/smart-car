...
void main(void)
{
    ...
    while (1)
    {
        ...
        if (print_flag)
        {
            ...
            {
                uart_dbg_div = 0;
                sprintf(debug_str, "fps=%u,off=%d,pre=%d,conf=%u,sc=%d,st=%d,gz=%d,sp=%d\r\n", current_fps, camera_bias_raw, camera_preview_raw, camera_confidence, control_debug_curve_x100, control_debug_steer_out, control_debug_gyro_z, control_debug_speed_x100);
