...
#define CAMERA_OUTPUT_GUARD_CONF_TH      45  // 低于此置信度时限制off/pre跳变，防止十字/圆弯误线导致舵机抽动
#define CAMERA_OUTPUT_HARD_CONF_TH       20  // 极低置信度时进一步收紧输出
#define CAMERA_OUTPUT_BIAS_STEP          8   // 低置信off每帧最大变化，太大会晃，太小会救车慢
#define CAMERA_OUTPUT_PREVIEW_STEP       12  // 低置信pre每帧最大变化
#define CAMERA_OUTPUT_HARD_BIAS_STEP     4
#define CAMERA_OUTPUT_HARD_PREVIEW_STEP  7
...
static int16 last_output_bias = 0;
static int16 last_output_preview = 0;
...
static int limit_step_i(int target, int last, int step)
{
...
uint8 find_mid_line_weight(void)
{
    ...
    {
        last_output_bias = bias_tmp;
        last_output_preview = preview_out;
        output_guard_init = 1;
    }
    else
    {
        if (camera_confidence < CAMERA_OUTPUT_GUARD_CONF_TH)
        {
            int bias_step;
            int preview_step;
            bias_step = (camera_confidence < CAMERA_OUTPUT_HARD_CONF_TH) ? CAMERA_OUTPUT_HARD_BIAS_STEP : CAMERA_OUTPUT_BIAS_STEP;
            preview_step = (camera_confidence < CAMERA_OUTPUT_HARD_CONF_TH) ? CAMERA_OUTPUT_HARD_PREVIEW_STEP : CAMERA_OUTPUT_PREVIEW_STEP;
            bias_tmp = (int16)limit_step_i((int)bias_tmp, (int)last_output_bias, bias_step);
            preview_out = (int16)limit_step_i((int)preview_out, (int)last_output_preview, preview_step);
        }
        last_output_bias = bias_tmp;
        last_output_preview = preview_out;
    }
    final_mid_tmp = (uint8)clamp_i(MID_W + (int)bias_tmp, 1, MT9V03X_W - 2);
